...
const tob64u = (str: string): string => {
 ...
 */
export const genDeterministicKeyPair = (seed: string) => {
 ...
 */
export const genDeterministicSEAPair = (seed: string) => {
 ...
 */
export const gunAuth = async (gunInstance: any, seed: string) => {
  const user = gunInstance.user();
  const login = new Promise((res, rej) => {
    user.auth(genDeterministicSEAPair(seed), (authres: any) => {
